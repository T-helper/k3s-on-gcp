...
resource "google_compute_region_backend_service" "k3s-vpnservers-tcp-external" {
  ...
  backend {
    group = google_compute_region_instance_group_manager.k3s-vpnservers.instance_group
  }
}
...
resource "google_compute_forwarding_rule" "k3s-vpnservers-udp-external" {
  ...
  load_balancing_scheme = "EXTERNAL"
  ip_address            = google_compute_address.k3s-vpn.address
  backend_service       = google_compute_region_backend_service.k3s-vpnservers-udp-external.id
  ports                 = [443]
  ip_protocol           = "UDP"
}